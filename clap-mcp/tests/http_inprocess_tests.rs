...
#[clap_mcp_output_from = "run_elicit_http_test"]
#[command(name = "elicit-http-test-cli", subcommand_required = false)]
enum ElicitHttpTestCli {
    ConfirmEcho {
        #[arg(long)]
...
fn run_elicit_http_test(cmd: ElicitHttpTestCli) -> String {
    match cmd {
        ElicitHttpTestCli::ConfirmEcho { message } => message.unwrap_or_else(|| "unused".into()),
    }
}

#[cfg(feature = "elicitation")]
#[derive(Clone, Default)]
struct DeclineElicitationHandler;

#[cfg(feature = "elicitation")]
impl ClientHandler for DeclineElicitationHandler {
...
#[cfg(feature = "elicitation")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn http_inprocess_elicitation_decline_is_handled() {
    use clap_mcp::ClapMcpServeOptions;
    ...
    let server = tokio::spawn(async move {
        let _ = ServeMcpBuilder::for_cli::<ElicitHttpTestCli>(McpListen::Http(addr))
    ...
    wait_for_http(addr).await;
    let uri = format!("http://{addr}/mcp");
    let client = DeclineElicitationHandler
        .serve(StreamableHttpClientTransport::from_uri(uri))
        .await
        .expect("connect");
