...
| Tokio runtime | `share_runtime` affects HTTP the same as stdio (multi-thread runtime when `reinvocation_safe` + `share_runtime`) |

## Low-level embed API

For servers that build schema JSON without the derive path:
