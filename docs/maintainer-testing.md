...
| Numeric `inputSchema` types (`input_type`, inference, flatten forward, globals after `Command::build`) | Schema property types match metadata; same-id isolation; custom `value_parser` stays string | `test_num_args_before_value_parser_disables_numeric_inference`, `test_struct_root_numeric_metadata_uses_clap_command_name`, `test_flatten_args_metadata_forwards_input_types`, `test_struct_root_flatten_args_metadata_forwards_input_types`, `test_global_numeric_type_survives_command_build`, `test_nonglobal_root_numeric_id_does_not_leak_to_child_string_id`, `test_global_string_id_does_not_inherit_sibling_integer_metadata`, `example_contract_vec_and_flags_*` |
| Clap `hide` vs MCP catalog / injected help | `hide(true)` stays in tools; app `help` kept; clap-injected `help` omitted | `test_hide_true_subcommand_stays_in_mcp_catalog`, `test_application_help_subcommand_stays_in_mcp_catalog` |
