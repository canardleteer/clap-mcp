...
cargo test -p clap-mcp --test input_schema_validation_tests
cargo test -p clap-mcp --test config_tests skip_flattened
cargo test -p clap-mcp --test config_tests nested_flatten
cargo test -p clap-mcp --test config_tests flatten_ordinary_local_size
cargo test -p clap-mcp --test config_tests flatten_meta_local_size
...
| Nested `serialize_topic` in flattened `Args` | Type-level `#[clap_mcp(args_metadata)]` + merge into `serialize_topic_args` (field-level not required) | `test_nested_flatten_args_serialize_topic_*`, `test_type_level_flatten_serialize_topic_keeps_case_insensitive_lock_keys`, `tests/ui/pass/serialize_topic_flattened_args.rs` |
...
| Numeric `inputSchema` types (`input_type`, inference, flatten forward, globals after `Command::build`) | Schema property types match metadata; same-id isolation; custom `value_parser` stays string | `test_num_args_before_value_parser_disables_numeric_inference`, `test_struct_root_numeric_metadata_uses_clap_command_name`, `test_flatten_args_metadata_forwards_input_types`, `test_struct_root_flatten_args_metadata_forwards_input_types`, `test_unnamed_struct_root_flatten_args_metadata_uses_clap_command_name`, `test_relative_path_flatten_args_metadata_forwards_input_types`, `test_global_numeric_type_survives_command_build`, `test_nonglobal_root_numeric_id_does_not_leak_to_child_string_id`, `test_child_declared_global_string_id_does_not_inherit_root_integer`, `test_child_local_size_override_does_not_inherit_root_global_integer`, `test_child_global_size_override_does_not_inherit_root_integer_on_descendants`, `test_serialized_local_arg_with_plain_flatten_options_compiles`, `test_global_string_id_does_not_inherit_sibling_integer_metadata`, `example_contract_vec_and_flags_*` |
| Arg ownership matrix (flatten + repeated ids + custom parser) | Native clap accepts `8MiB`; `tools/list` type matches the owning arg, fresh and `build()`; ordinary `Args` and `args_metadata`; root global and non-global; child local and global (plus `inspect`); one-level and two-level flatten; skip flatten hides `size`; struct-root flatten stays string; `hide_possible_values` is not ownership; live-root declaration remap; imperative local boundary and two-level inherit; `from_global` inherits | `test_flatten_ordinary_local_size_vs_root_global_integer`, `test_flatten_ordinary_global_size_vs_root_global_integer_on_descendants`, `test_flatten_ordinary_local_size_vs_root_nonglobal_integer`, `test_flatten_ordinary_global_size_vs_root_nonglobal_integer_on_descendants`, `test_flatten_meta_local_size_vs_root_global_integer`, `test_flatten_meta_global_size_vs_root_global_integer_on_descendants`, `test_flatten_meta_local_size_vs_root_nonglobal_integer`, `test_flatten_meta_global_size_vs_root_nonglobal_integer_on_descendants`, `test_two_level_flatten_ordinary_size_vs_root_global_integer`, `test_two_level_flatten_meta_size_vs_root_nonglobal_integer`, `test_hide_possible_values_on_inherited_global_is_not_ownership`, `test_skip_flatten_lexical_size_is_not_advertised`, `test_struct_root_flatten_lexical_size_is_string`, `test_imperative_declared_arg_id_blocks_parent_integer`, `test_imperative_empty_declaration_map_inherits_parent_global_integer`, `test_unnamed_struct_root_global_workers_integer_on_descendants`, `test_imperative_root_global_integer_survives_two_subcommand_levels`, `test_imperative_child_local_size_does_not_inherit_root_integer`, `test_imperative_child_global_lexical_override_uses_declared_arg_id`, `test_from_global_does_not_infer_integer_over_lexical_root` |
| Clap `hide` vs MCP catalog / injected help | `hide(true)` stays in tools; app `help` kept; clap auto-help omitted (including already-built `Command`) | `test_hide_true_subcommand_stays_in_mcp_catalog`, `test_application_help_subcommand_stays_in_mcp_catalog`, `test_already_built_command_omits_auto_help_from_mcp_catalog` |
| Nested enum name matching variant + numeric metadata | Compiles without `CommandFactory`; leaf types advertise | `test_schema_only_enum_matching_variant_name_compiles_with_numeric_types`, `tests/ui/pass/schema_only_matching_variant_name.rs` |
...
| Command | Use |
| --- | --- |
| `cargo xtask conformance` | Dual pass (previous `2025-11-25` + current released `2026-07-28`); start → test → stop |
| `cargo xtask conformance --suite active` | Previous-release pass only (`2025-11-25`) |
